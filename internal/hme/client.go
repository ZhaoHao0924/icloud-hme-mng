...
// SessionCookies 返回当前会话 Cookie 的副本。
func (c *Client) SessionCookies() map[string]string {
...
func requestLogURL(rawURL string) string {
...
func (c *Client) request(method, rawURL string, body any, timeout time.Duration, maxAttempts int) (string, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if c.Verbose {
			...
			c.log(">>> %s %s", method, requestLogURL(fullURL))
			c.log(">>> Cookie 数量: %d", len(c.Cookies))
			c.log(">>> 请求头: %s", strings.Join(headerNames, ", "))
		}
