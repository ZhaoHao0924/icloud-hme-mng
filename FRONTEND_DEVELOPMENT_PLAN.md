...
| Phase 8 操作审计、移动端适配与错误体验 | 进行中 | 2026-08-14 | - | `AUD-001`、`QA-009`、`ERR-001` 与 `QA-011` 已完成审计安全回归及 Apple 上游错误契约；`QA-010` 已完成账户、凭据、别名、收件箱和设置页面及关键对话框的四视口 Playwright 验收基线；iOS Safari 与 Android Chrome 真机或云设备冒烟仍待完成。FE-601 仍待开发；本次 Go 验证待推送 CI 完成。 |
...
| 日期 | 变更 | 原因 |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-14 | 完成 `ERR-001` 与 `QA-011`：新增隐私安全的 Apple 上游错误分类和稳定业务错误契约，按已确认会话失效、资格、别名额度、频率/风控、设备信任、参数/状态冲突、服务不可用和未知拒绝分别返回 `code`、`action` 与 `retryable`；别名、收件箱、自动化和密码登录均接入，前端只会对 `icloud_session_expired` 打开 Cookie 恢复入口。新增 Mock、单元和浏览器回归，覆盖“列表可读但创建 `generate` 返回普通 `403`”不误报 Cookie 失效。验证通过：`npm run lint`、`npm run typecheck`、`npm run test`（174/174）、`npm run test:e2e:mock`（41/41）、`npm run build`、相关 Prettier 检查和 `git diff --check`；本机未安装 Go 工具链，Go 单测、竞态和构建待本次推送 CI。 | 将 Apple 上游拒绝从基于状态码的猜测改为可验证的错误边界，避免错误引导用户反复更新 Cookie，同时保留可恢复的服务异常重试策略。 |
