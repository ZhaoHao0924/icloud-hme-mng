...
| Phase 8 操作审计、移动端适配与错误体验 | 进行中 | 2026-08-14 | - | `AUD-001`、`QA-009`、`ERR-001` 与 `QA-011` 已完成审计安全回归及 Apple 上游错误契约；`QA-010` 已完成账户、凭据、别名、收件箱和设置页面及关键对话框的四视口 Playwright 验收基线；iOS Safari 与 Android Chrome 真机或云设备冒烟仍待完成。FE-601 仍待开发；GitHub Actions Run 48 已通过 Linux 竞态、前端回归、发布二进制与 Docker 烟测。 |
