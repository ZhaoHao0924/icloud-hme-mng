...
> 创建日期：2026-07-31
>
> 当前阶段：既有 Phase 0 至 Phase 8 已全部完成；后续需求另行建立新阶段
...
### Phase 8 - 操作审计、移动端适配与错误体验（已完成）

- [x] AUD-001 扩展所有操作记录的请求与响应审计。为 API 操作统一记录操作时间、操作类型、关联请求 ID、HTTP 方法、实际路径、原始 query、路径参数、完整请求体、管理端原始响应体、HTTP 状态、业务错误码、耗时和重试次数；定时任务继续记录结构化执行统计。请求/响应 header 不写入，避免重复保存 Bearer Token 和平台会话 Cookie；管理端响应快照不等同于 Apple 内部上游 HTTP 原文。定义日志 schema v3、7 天保留周期、查询 API/UI 和 v1/v2 兼容策略，并补充原文持久化、失败路径、大体积记录和并发写入测试。
...
- [x] QA-009 为 AUD-001 建立审计契约回归：覆盖成功、参数校验失败、Apple 上游拒绝、超时和定时任务；断言 API 请求参数原值与管理端原始响应可从数据文件、查询 API 和设置页详情中完整回放，header 凭据与 Apple 内部上游原文不会被额外捕获，`/api/logs` 不递归记录，且 v1/v2 日志仍可读取。
- [x] QA-010 为 FE-601 建立移动端验收：`web/e2e-mock/mobile-acceptance.spec.ts` 已在 `375 x 667`、`390 x 844`、`768 x 1024` 和桌面基线覆盖账户添加、别名创建、凭据更新、收件箱筛选和设置日志入口的横向溢出、可达性及对话框键盘焦点恢复；线上环境持续运行一段时间后未发现移动端焦点、软键盘、布局或关键提交操作问题，以自动化回归和线上实际使用结果完成验收。
...
完成门槛：每条可审计 API 操作均有结构化、可查询且可回放原始请求参数与管理端响应的记录，日志目录按密钥材料保护；核心管理流程在定义的移动端浏览器和视口下可完整完成；Apple 上游错误具有可验证、可恢复且不误导用户的差异化提示。

## 11. 测试策略

| 层级 | 工具 | 覆盖重点 |
| ------------ | --------------------- | ------------------------------------------ |
| Go 单元测试 | `go test` | 配置、审计原文快照、参数边界、邮件解析、登录状态机 |
...
| 操作日志包含敏感原文 | 数据目录泄漏会暴露账户和邮件信息 | 默认同源鉴权、7 天保留、日志文件权限保护、header 凭据不捕获、数据目录禁止公开或提交 Git |
...
- 密码和 Cookie 不进入 URL、localStorage、截图、测试 fixture 或版本库；操作日志是明确例外，按需求保留请求体原值并要求保护数据目录。
...
| ------------------ | ------ | ---------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase 0 后端整改 | 已完成 | 2026-07-31 | 2026-08-16 | BE-001 至 BE-010 已完成；GitHub Actions Run 25 已通过 Linux `go test -race ./...`，受控线上环境持续运行期间未发现 Apple Cookie、IMAP、Web 或 2FA 阻断问题，外部验收完成 |
...
| Phase 3 凭据与登录 | 已完成 | 2026-08-01 | 2026-08-16 | FE-201 至 FE-206 已完成并通过自动化安全审计；Cookie、App Password、无 2FA 与 2FA 登录流程已通过受控线上环境持续运行验收，未发现阻断问题 |
...
| Phase 6 生产集成 | 已完成 | 2026-08-01 | 2026-08-16 | FE-501 至 FE-507 已完成；Windows 原生发布烟测已验证嵌入资源、鉴权、缓存、安全头、SPA fallback 与重启持久化，GitHub Actions 已通过 Bash 构建、Linux 发布二进制和 Docker 挂载数据目录烟测；结合线上环境持续稳定运行结果，生产集成外部验收完成 |
| Phase 7 发布验收 | 已完成 | 2026-08-01 | 2026-08-14 | QA-001 至 QA-008 已完成并保留自动化及三视口视觉证据；QA-008 已在线上受控账户环境完成，未记录账号、凭据或原始响应 |
| Phase 8 操作审计、移动端适配与错误体验 | 已完成 | 2026-08-14 | 2026-08-16 | `AUD-001`、`QA-009`、`QA-010`、`ERR-001`、`QA-011` 与 `FE-601` 均已完成。移动端已覆盖账户、凭据、别名、收件箱、设置和操作日志页面，关键对话框依据安全区域和 `visualViewport` 内部滚动；四个视口的 Playwright 回归验证无横向溢出和关键操作可达，线上环境持续运行一段时间未发现移动端关键流程问题；GitHub Actions Run 48 已通过 Linux 竞态、前端回归、发布二进制与 Docker 烟测。 |

当前开发结项（2026-08-16）：`BE-001` 至 `BE-010`、Phase 1 至 Phase 6 的全部计划内 FE 任务、`FE-601`、QA-001 至 QA-011、`AUD-001` 与 `ERR-001` 均已完成；Phase 0 至 Phase 8 的自动化、发布运行时、真实账户、移动端和线上持续运行验收全部结项。GitHub Actions 已通过 Linux 竞态、前端回归、发布二进制与 Docker 烟测，Windows 原生发布烟测已覆盖嵌入资源、鉴权、SPA fallback、缓存、安全头、404 和重启持久化，线上环境持续运行期间未发现阻断问题。本轮开发阶段正式结束；后续新增需求应先定义范围并建立新的 Phase，不再追加到本轮已结项阶段。

## 15. 决策与变更记录

| 日期 | 变更 | 原因 |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-16 | 完成 Phase 0、Phase 3 与 Phase 6 的剩余外部验收，并确认 Phase 0 至 Phase 8 全部结项；线上环境持续运行期间未发现 Apple 凭据、登录、邮件、发布运行时或移动端关键流程问题。后续需求不再追加到本轮计划，而是在范围明确后建立新的开发阶段。 | 现有功能已经通过自动化、发布烟测和线上持续运行验证，继续保留“进行中”状态已不能反映实际交付情况。 |
| 2026-08-16 | 完成 `QA-010`：保留四个定义视口的 Playwright 移动端回归，并以线上环境持续运行一段时间、未发现移动端焦点、软键盘、横向溢出或关键提交操作问题作为最终验收依据；Phase 8 状态更新为已完成。 | 线上实际使用结果已经覆盖发布后的稳定性观察，可与既有自动化证据共同完成移动端验收。 |
| 2026-08-16 | 将操作日志升级为 schema v3：保存 HTTP 方法、实际路径、原始 query、路径参数、完整请求体和管理端原始响应体；UTF-8 原文直存、二进制使用 Base64，保留 v1/v2 读取兼容，并在设置页提供可展开的请求/响应详情。请求/响应 header 仍不记录，`/api/logs` 不递归记录，既有 1 MiB 请求体限制不变。 | 项目为个人使用的管理端，排障和复现价值高于操作日志字段级脱敏要求；通过缩短保留周期和保护数据目录承担相应风险。 |
