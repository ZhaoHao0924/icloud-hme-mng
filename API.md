...
该接口使用已保存的 163 发件、QQ 收件配置发送测试邮件。自动化通知使用有界异步队列，SMTP 失败最多重试 3 次，发送失败或队列已满不会阻塞别名任务。通知事件包括手动/定时自动化结果、自动暂停和 iCloud 会话失效。

邮件以 `multipart/alternative` 发送，同时提供纯文本与 HTML 正文（两者均为 base64 编码的 UTF-8），并带有 `Date`、`Message-ID` 和 `Auto-Submitted: auto-generated` 头。主题格式为 `[iCloud HME] 事件类型 · 状态 · 新增 x/y · 账户 xxx...xxx`，正文按「事件概要 / 创建统计 / 补充信息」分段展示事件类型、执行状态、触发方式、脱敏账户标识、发生时间、数量统计、暂停原因、错误摘要和建议操作，机器枚举值会同时给出中文说明。正文仍然只包含上述脱敏字段，不含授权码、Cookie、密码、别名地址或邮件内容。
